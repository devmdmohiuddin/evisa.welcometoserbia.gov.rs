...
const UpdateUser = () => {
  ...
  const {
    ...
    userError,
    isUserLoading,
  } = useSWR(`/api/auth/users/${id}`, fetcher);
  ...
  console.log("update", userData)
    ...
    useEffect(() => {
        if (userData) {
          ...
          setValue("requestNum", userData.requestNum);
          setValue("visaReqId", userData.visaReqId);
          setValue("foreignRegNum", userData.foreignRegNum);
          setValue("citizenship", userData.citizenship);
          setValue("status", userData.status);
          setValue("comment", userData.comment);
          setValue("travelPurpose", userData.travelPurpose);
          setValue("gender", userData.gender);
          setValue("lastNameAtBirth", userData.lastNameAtBirth);
          setValue("dateOfBirth", userData.dateOfBirth.slice(0, 10));
          ...
          setValue("issuingCountry", userData.issuingCountry);
          setValue("maritalStatus", userData.maritalStatus);
          setValue("fatherName", userData.fatherName);
          setValue("motherName", userData.motherName);
          setValue("currentProfession", userData.currentProfession);
          setValue("employerCompany", userData.employerCompany);
          setValue("employerAddress", userData.employerAddress);
          setValue("employerTelephone", userData.employerTelephone);
          setValue("typeOfTravelDoc", userData.typeOfTravelDoc);
          setValue("travelDocNum", userData.travelDocNum);
          setValue("issuedBy", userData.issuedBy);
          setValue("dateOfIssue", userData.dateOfIssue.slice(0, 10));
          setValue("validUntil", userData.validUntil.slice(0, 10));
          setValue("countryOfOrigin", userData.countryOfOrigin);
          setValue("diplomaticAndConsular", userData.diplomaticAndConsular);
          setValue("visaType", userData.visaType);
          setValue("travelPurpose2", userData.travelPurpose2);
          setValue("numberOfDays", userData.numberOfDays);
          setValue("otherVisaIssues", userData.otherVisaIssues);
          setValue("dateOfArrival", userData.dateOfArrival.slice(0, 10));
          setValue("dateOfDeparture", userData.dateOfDeparture.slice(0, 10));
          setValue("borderCrossing", userData.borderCrossing);
          setValue("meansOfTransport", userData.meansOfTransport);
          setValue("previouslyStayed", userData.previouslyStayed);
          setValue("hostFirstName", userData.hostFirstName);
          setValue("hostTelNum", userData.hostTelNum);
          setValue("hostAddress", userData.hostAddress);
          setValue("hostEmail", userData.hostEmail);
          setValue("municipality", userData.municipality);
          setValue("settlement", userData.settlement);
          setValue("street", userData.street);
          setValue("houseNum", userData.houseNum);
          setValue("entrance", userData.entrance);
          setValue("floor", userData.floor);
          setValue("apartment", userData.apartment);
          setValue("travelCost", userData.travelCost);
          setValue("livingExpenses", userData.livingExpenses);
        }
      }, [userData, setValue]);
    ...
    const submitHandler = async (data) => {
        try {
            ...
            if (response.ok) {
              ...
              toast.error("Something went wrong to update user.");
            }
          } catch (error) {
                ...
                <form onSubmit={handleSubmit(submitHandler)} className="sm:grid grid-cols-2 gap-6 space-y-3 sm:space-y-0">
                    ...
                    <div className="space-y-1">
                        ...
                        <select name="borderCrossing" id="borderCrossing" className={`w-full border-2 rounded-md p-3 ${
                            ...
                            <option value="ĆIRPANOVA">NOVI SAD</option>
